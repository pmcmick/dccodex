#!/usr/bin/env python3
"""Stage and optionally package the DCCodex npm module."""
...
DCCODEX_NPM_NAME = "@pmcmick/dccodex"

# `npm_name` is the local optional-dependency alias consumed by `bin/dccodex.js`.
# The underlying package published to npm is always `@pmcmick/dccodex`.
DCCODEX_PLATFORM_PACKAGES: dict[str, dict[str, str]] = {
    "dccodex-linux-x64": {
        "npm_name": "@pmcmick/dccodex-linux-x64",
        "npm_tag": "linux-x64",
    ...
    "dccodex-linux-arm64": {
        "npm_name": "@pmcmick/dccodex-linux-arm64",
        "npm_tag": "linux-arm64",
    ...
    "dccodex-darwin-x64": {
        "npm_name": "@pmcmick/dccodex-darwin-x64",
        "npm_tag": "darwin-x64",
        "target_triple": "x86_64-apple-darwin",
        "os": "darwin",
        "cpu": "x64",
    },
    "dccodex-darwin-arm64": {
        "npm_name": "@pmcmick/dccodex-darwin-arm64",
    ...
    "dccodex-win32-x64": {
        "npm_name": "@pmcmick/dccodex-win32-x64",
        "npm_tag": "win32-x64",
    ...
    "dccodex-win32-arm64": {
        "npm_name": "@pmcmick/dccodex-win32-arm64",
        "npm_tag": "win32-arm64",
...
PACKAGE_EXPANSIONS: dict[str, list[str]] = {
    "dccodex": ["dccodex", *DCCODEX_PLATFORM_PACKAGES],
}

PACKAGE_NATIVE_COMPONENTS: dict[str, list[str]] = {
    "dccodex": [],
    "dccodex-linux-x64": ["codex", "rg"],
    "dccodex-linux-arm64": ["codex", "rg"],
    "dccodex-darwin-x64": ["codex", "rg"],
    "dccodex-darwin-arm64": ["codex", "rg"],
    "dccodex-win32-x64": ["codex", "rg", "codex-windows-sandbox-setup", "codex-command-runner"],
    "dccodex-win32-arm64": ["codex", "rg", "codex-windows-sandbox-setup", "codex-command-runner"],
...
PACKAGE_TARGET_FILTERS: dict[str, str] = {
    ...
    for package_name, package_config in DCCODEX_PLATFORM_PACKAGES.items()
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build or stage the DCCodex npm package.")
    parser.add_argument(
        "--package",
        choices=PACKAGE_CHOICES,
        default="dccodex",
        help="Which npm package to stage (default: dccodex).",
    )
    parser.add_argument(
        "--version",
        ...
        if release_version:
            ...
            if package == "dccodex":
                print(
                    ...
                    "Verify the DCCodex CLI:\n"
                    f"    node {staging_dir_str}/bin/dccodex.js --version\n"
                    f"    node {staging_dir_str}/bin/dccodex.js --help\n\n"
                )
            ...
            elif package in DCCODEX_PLATFORM_PACKAGES:
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
...
def prepare_staging_dir(staging_dir: Path | None) -> tuple[Path, bool]:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="dccodex-npm-stage-"))
...
def stage_sources(staging_dir: Path, version: str, package: str) -> None:
    ...
    if package == "dccodex":
        ...
        shutil.copy2(CODEX_CLI_ROOT / "bin" / "dccodex.js", bin_dir / "dccodex.js")
        ...
        platform_package = DCCODEX_PLATFORM_PACKAGES[package]
    ...
    if package == "dccodex":
        ...
        package_json["optionalDependencies"] = {
            DCCODEX_PLATFORM_PACKAGES[platform_package]["npm_name"]: (
                f"npm:{DCCODEX_NPM_NAME}@"
                f"{compute_platform_package_version(version, DCCODEX_PLATFORM_PACKAGES[platform_package]['npm_tag'])}"
            )
            for platform_package in PACKAGE_EXPANSIONS["dccodex"]
            if platform_package != "dccodex"
        }
        ...
        if not isinstance(dependencies, dict):
            dependencies = {}
        dependencies[DCCODEX_NPM_NAME] = version
...
def run_npm_pack(staging_dir: Path, output_path: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="dccodex-npm-pack-") as pack_dir_str:
